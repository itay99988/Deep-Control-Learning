...
from utils import print_experiment, EXP_PATH
...
class BlackboxEnvironment(DistSystem):
    ...
    def export_results(self, directory):
        output_file_name = f'{directory}/{datetime.utcnow().strftime("%d%m%Y-%H%M%S%f")[:-3]}.txt'
        print_experiment(output_file_name, self.system_execution, self.environment_execution)
